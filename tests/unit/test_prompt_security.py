...
def test_build_prompt_generates_resume_pdf_if_missing(monkeypatch, tmp_path):
    resume_txt = tmp_path / "tailored_resume.txt"
    resume_pdf = tmp_path / "tailored_resume.pdf"
    resume_txt.write_text("Tailored resume text", encoding="utf-8")
    ...
    def _fake_convert_to_pdf(path):
        assert path == resume_txt
        resume_pdf.write_bytes(b"%PDF-1.4\n%%EOF")
        return resume_pdf

    monkeypatch.setattr(pdf_mod, "convert_to_pdf", _fake_convert_to_pdf)
    ...
    assert "Resume PDF (upload this):" in prompt
    assert (tmp_path / "apply-workers" / "current" / "Test_User_Resume.pdf").exists()
